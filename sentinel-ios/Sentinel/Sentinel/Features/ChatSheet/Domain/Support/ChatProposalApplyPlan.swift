...
extension ChatSheetState.Message.SuggestionsPayload {
    var conflictDrafts: [CalendarSyncClient.Draft] {
        suggestions.compactMap { suggestion in
            guard suggestion.status == .pending,
                  let startAt = suggestion.startAt else {
                return nil
            }
    ...
    func applyingSelectionPlan() -> ChatProposalApplyPlan? {
        ...
        let acceptedSuggestions: [ChatSheetState.Suggestion]

        if pendingSuggestions.count == 1 {
            acceptedSuggestions = pendingSuggestions
        } else {
        ...
        let acceptedIndices = acceptedSuggestions
            .map(\.actionIndex)
            .sorted()

        guard !acceptedIndices.isEmpty else {
            return nil
        }

        let datedSuggestions = acceptedSuggestions.filter { $0.action != .delete }
        let startDates = datedSuggestions.compactMap(\.startAt)
        let endDates = datedSuggestions.compactMap { suggestion in
        ...
        if let earliestStart = startDates.min(),
           let latestEnd = endDates.max() {
            syncRange = earliestStart...latestEnd
        } else {
            syncRange = nil
        }
        ...
        return ChatProposalApplyPlan(
            ...
            upsertEventIDs: Set(
                acceptedSuggestions
                    .filter { $0.action == .update }
                    .compactMap(\.eventId)
            )
        )
    }

    func updatingConflicts(_ conflicts: [ChatSheetState.Suggestion.ID: Bool]) -> Self {
