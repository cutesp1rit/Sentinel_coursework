...
struct ChatThreadFeature {
    ...
    var body: some Reducer<State, Action> {
        CombineReducers {
            Scope(state: \.attachmentPicker, action: \.attachmentPicker) {
            ...
            Reduce { state, action in
                ...
                case .attachmentButtonTapped:
                    ...
                    return .merge(
                        .send(.delegate(.attachmentFlowRequested)),
                        .send(.attachmentPicker(.presenterTapped))
                    )
                ...
                case let .attachmentRemoved(attachmentID):
                    ...
                    state.recentPhotoAttachmentIDs = state.recentPhotoAttachmentIDs.filter { $0.value != attachmentID }
                    return .none
                ...
                case let .recentPhotoAttachmentLoaded(photoID, attachment):
                    guard let attachment else {
                        state.attachmentPicker.selectedRecentPhotoIDs.removeAll { $0 == photoID }
                        return .none
                    }
                    state.recentPhotoAttachmentIDs[photoID] = attachment.id
                ...
                case let .attachmentPicker(.delegate(.recentPhotoSelected(photoID))):
                    let nextIndex = state.composerAttachments.count
                    return .run { [chatAttachmentLibraryClient] send in
                        await send(.recentPhotoAttachmentLoaded(photoID, await chatAttachmentLibraryClient.makeAttachmentFromRecentPhoto(photoID, nextIndex)))
                    }

                case let .attachmentPicker(.delegate(.recentPhotoDeselected(photoID))):
                    if let attachmentID = state.recentPhotoAttachmentIDs.removeValue(forKey: photoID) {
                        state.composerAttachments.removeAll { $0.id == attachmentID }
                    }
                    return .none
