...
public struct RebalanceDayInput: Equatable, Sendable {
    public let date: Date
    public let resourceBattery: Double?

    public init(date: Date, resourceBattery: Double?) {
        self.date = date
        self.resourceBattery = resourceBattery
    }
}

public struct RebalanceProposedEvent: Equatable, Identifiable, Sendable {
    public let id: UUID
    public let title: String
    public let startAt: Date
    public let endAt: Date?
    public let originalStartAt: Date
    public let originalEndAt: Date?
    ...
    public init(
        ...
        self.originalEndAt = originalEndAt
        self.changed = changed
    }
}

public struct RebalancePreview: Equatable, Sendable {
    public let proposed: [RebalanceProposedEvent]
    public let summary: String
    public let changedCount: Int
    public let unchangedCount: Int

    public init(proposed: [RebalanceProposedEvent], summary: String, changedCount: Int, unchangedCount: Int) {
...
public struct RebalanceApplyEvent: Equatable, Sendable {
    public let id: UUID
    public let startAt: Date
    public let endAt: Date?

    public init(id: UUID, startAt: Date, endAt: Date?) {
        self.id = id
        self.startAt = startAt
        self.endAt = endAt
    }
...
public struct RebalanceClient: Sendable {
    public var apply: @Sendable (_ events: [RebalanceApplyEvent], _ bearerToken: String) async throws -> Void
    public var propose: @Sendable (_ timezone: String, _ days: [RebalanceDayInput], _ userPrompt: String?, _ bearerToken: String) async throws -> RebalancePreview

    public init(
        apply: @escaping @Sendable (_ events: [RebalanceApplyEvent], _ bearerToken: String) async throws -> Void,
        propose: @escaping @Sendable (_ timezone: String, _ days: [RebalanceDayInput], _ userPrompt: String?, _ bearerToken: String) async throws -> RebalancePreview
    ) {
        self.apply = apply
...
extension RebalanceClient: DependencyKey {
    public static let liveValue = RebalanceClient(
